...
\large A module for Perelman's finite extinction argument}
...
flow existence or finite extinction. We explain the corrected curvature
estimate, the use of a spatial time-dependent connection, the
...
ambient Ricci flow \cite{perelman2003}. The relevant estimates are detailed in
Chapter 19 of \emph{Ricci Flow and the Poincar\'e Conjecture} \cite{ricciflow2007};
the subsequent correction \cite{curveestimates2015} changes the curvature
evolution and the dependence on initial data. The formalization reported here
concerns this estimate module. It supplies
...
The correction to Section 19.2 \cite{curveestimates2015} identifies the omitted
time variation of the ambient connection. It produces an error linear in $k$,
as well as quadratic terms, and consequently a total-curvature estimate
involving initial length. The implementation retains this term but uses a
...
\cite{mathlib,frenzymath}. The project imports the upstream Ricci-flow
and Riemannian geometry libraries. These libraries provide the smooth Riemannian
...
Project declaration names in this report are fully qualified; the upstream
Ricci-flow predicate is written as \path{IsRicciFlowOn}. The main product
...
$a,b:\R$, \path{IsRicciFlowOn g (Set.Icc a b)}, $a\le b$, and
...
\path{IsRicciFlowOn} has four fields: order-connectedness and
